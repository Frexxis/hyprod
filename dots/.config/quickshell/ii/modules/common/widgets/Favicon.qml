...
IconImage {
    ...
    Process {
        ...
        running: false
        command: ["bash", "-c", `[ -f '${StringUtils.shellSingleQuoteEscape(faviconFilePath)}' ] || curl -s '${root.faviconUrl}' -o '${StringUtils.shellSingleQuoteEscape(faviconFilePath)}' -L -H 'User-Agent: ${StringUtils.shellSingleQuoteEscape(downloadUserAgent)}'`]
