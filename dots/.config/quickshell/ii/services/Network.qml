...
Singleton {
    ...
    property WifiAccessPoint pendingReconnectTarget: null
    ...
    function changePassword(network: WifiAccessPoint, password: string, username = ""): void {
        ...
        root.pendingReconnectTarget = network;
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => { // Re-attempt connection after changing password
            if (exitCode !== 0) return;
            reconnectAfterPasswordTimer.restart();
        }
    }

    Timer {
        id: reconnectAfterPasswordTimer
        interval: 500
        repeat: false
        onTriggered: {
            if (!root.pendingReconnectTarget) return;
            root.connectToWifiNetwork(root.pendingReconnectTarget);
            root.pendingReconnectTarget = null;
        }
    }
